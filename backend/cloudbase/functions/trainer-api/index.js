...
async function createIOSBindingCode(request) {
  const auth = await requireIdentity(request.requestId)
  if (auth.error) return auth.error
  ...
  const result = await withIdempotency(
    request.requestId,
    'trainer.createIOSBindingCode',
    ownerId,
    ownerId,
    async () => {
      await db.collection('ios_account_links').add({
        data: {
          token,
          userId: ownerId,
          consumedAt: 0,
      ...
      return { token, expiresAt: now + 10 * 60 * 1000 }
    }
  )
  return ok(result, request.requestId)
}
...
function normalizePlanPayload(payload, status) {
  ...
  const phases = Array.isArray(payload.phases)
    ? payload.phases.map((phase, index) => ({
      ...
      id: typeof phase.id === 'string' && phase.id.trim() ? phase.id.trim() : '',
      ...
      reminders: Array.isArray(phase.reminders)
        ? phase.reminders.filter((item) => typeof item === 'string' && item.trim()).map((item) => item.trim())
        : [],
      activityNames: Array.isArray(phase.activityNames)
        ? phase.activityNames.filter((item) => typeof item === 'string' && item.trim()).map((item) => item.trim())
        : [],
...
async function upsertPlan(request, status) {
  ...
  const result = await withIdempotency(
    ...
    async () => {
      ...
      return { planId: created._id, status }
    }
  )
